...
def cmd_auth_url():
    ...
    print("  google_calendar_oauth.py exchange <code>")
    print()
...
def cmd_exchange(code: str):
    ...
    out = {
        ...
        "calendar_id": os.environ.get("AI_DISTRO_GOOGLE_CALENDAR_ID", "primary"),
    }
    path = oauth_path()
    ...
    with open(path, "w", encoding="utf-8") as fh:
        ...
        fh.write("\n")
    print(f"Wrote Google Calendar OAuth config to {path}")
    return 0


def main():
    if len(sys.argv) < 2:
        print("usage: google_calendar_oauth.py auth-url|exchange <code>", file=sys.stderr)
        return 2
    ...
    if cmd == "exchange":
        if len(sys.argv) < 3:
            print("usage: google_calendar_oauth.py exchange <code>", file=sys.stderr)
            return 2
